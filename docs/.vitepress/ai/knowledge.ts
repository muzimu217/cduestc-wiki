...
const TOPIC_ALIASES = [
    ...
    ['专升本', '升本', '专科升本科'],
    ['考研', '研究生', '读研', '硕士研究生'],
    ['入党', '党员', '入党申请书', '预备党员', '党组织'],
    ['征兵', '入伍', '当兵', '参军'],
    ['就业', '实习', '招聘', '求职', '三方协议', '秋招', '春招'],
    ['资助', '助学贷款', '助学金', '困难认定', '勤工助学', '绿色通道'],
]
...
function getQueryTerms(query: string): WeightedTerm[] {
    ...
    // 「问学院/问专业」类查询里的高频字会稀释目标页，按意图锚定到对应栏目页。
    if (/(?:有哪些|几个|全部|所有|介绍).{0,4}学院|学院情况|二级学院|院系设置/u.test(normalizedQuery)) {
        addTerm(terms, '学院情况', 10)
        addTerm(terms, '院系设置', 7)
    }
    if (/(?:有哪些|几个|全部|所有|什么|介绍).{0,4}专业|专业列表|专业设置/u.test(normalizedQuery)) {
        addTerm(terms, '专业列表', 10)
    }
